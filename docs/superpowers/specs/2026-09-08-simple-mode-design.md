...
**Status:** design approved, plans written (part1-deletion, part2-features), not yet implemented
**Date:** 2026-09-08
...
| 취소 | `⌘F2`, 실행 중이면 `⌘C` | |
| 테이블 미리보기 (`LIMIT 100`) | 테이블 탭에서 `↩` | 트리 더블클릭, 테이블 탭 클릭 |
| 셀 복사 | `⌘C` | |
...
트리의 테이블 노드를 더블클릭하면, 또는 테이블 탭의 항목을 클릭하거나
`↩`를 누르면:
...
트리의 단일 클릭과 `↩`는 지금처럼 컬럼을 펼친다. tview의 `TreeView`에는
노드를 펼치는 다른 키가 없어서, `↩`를 미리보기로 바꾸면 키보드만 쓰는
사람이 컬럼을 볼 길이 없어진다. 키보드 미리보기는 테이블 탭의 `↩`다.
컬럼 노드의 `↩`는 지금처럼 이름을 편집기에 넣는다.

### 결과 전체 복사

행 단위 복사는 결과 전체 복사로 갈음한다.
...
`dv`를 설정 파일 없이 실행하면, 또는 데이터소스가 둘 이상인데 이름 없이
실행하면, 세션 없이 데이터소스 목록만 있는 **런처**(`ui.Launch`)가 뜬다.
런처는 자기 `tview.Application`을 가진 작은 화면이고, 사용자가 연결하면
열린 세션을 돌려주며, 그 다음은 지금의 `ui.New(sess, ...)` 경로 그대로다.
`App`을 세션 없이 시작하게 만들지 않는 이유는, `App`의 거의 모든
메서드가 연결을 전제하고 그 전제를 하나씩 풀 때 생기는 nil 분기가 새
기능보다 크기 때문이다. 목록·폼 컴포넌트는 런처와 앱 안 다이얼로그가
같은 것을 쓴다.

데이터소스가 정확히 하나면 `dv`는 지금처럼 바로 연다. `dv open <name>`도
지금처럼 바로 연결하고, 실패하면 종료 코드로 말한다.
...
- `config`는 여전히 `io.Reader`로 파싱하고, `Save`는 `io.Writer`로
  ...
  나뉜다. 전자는 파일 없이 테스트한다.
- `ui`는 여전히 tview를 아는 유일한 패키지다. 런처는 `ui` 안의 함수이고,
  `App`은 계속 연결을 전제한다.
...
- 설정 없는 시작: 런처가 열리고, 닫으면 세션 없이 끝나며, 연결하면 세션을
  돌려준다.
...
- **런처 테스트.** 런처는 `App`의 하네스(`inspect`/`waitFor`)를 쓸 수
  없다. 시뮬레이션 스크린에 키를 넣고 종료 채널만 기다리는 방식이라,
  폼의 Tab 순서가 바뀌면 테스트가 함께 바뀐다.
